...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Check device capabilities for performance mode
    ...
    let lastFrameTime = 0;
    const targetFPS = isPerformanceModeEnabled ? 15 : 30; // Lower FPS for slower devices
    const frameInterval = 1000 / targetFPS;

    let debugFrameCount = 0;
    
    function updateFrame(currentTime = 0) {
      // Frame rate limiting
      ...
      // Skip rendering if canvas is not visible (performance optimization)
    ...
    function drawDisplay(target: any) {
      ...
      if (config.SHADING && (displayMaterial.uniforms as any).texelSize !== undefined)
        gl.uniform2f(
          (displayMaterial.uniforms as any).texelSize,
          1.0 / width,
          1.0 / height
        );
      if ((displayMaterial.uniforms as any).uTexture !== undefined)
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: any) {
      ...
      gl.uniform3f(splatProgram.uniforms.color, color.r || 0, color.g || 0, color.b || 0);
      blit(dye.write);
      dye.swap();
    }
    ...
    document.body.addEventListener(
      ...
      function handleFirstMouseMove(e) {
        // Check if first movement is over social links
        const elementUnderMouse = document.elementFromPoint(e.clientX, e.clientY);
        const isOverSocialLink = elementUnderMouse?.closest('.social-links a') || 
                                 elementUnderMouse?.closest('a[href*="github.com"]') ||
                                 elementUnderMouse?.closest('a[href*="linkedin.com"]');
    ...
    window.addEventListener("mousemove", (e) => {
      // Check if mouse is over social links (disable splash effects there)
      const elementUnderMouse = document.elementFromPoint(e.clientX, e.clientY);
      const isOverSocialLink = elementUnderMouse?.closest('.social-links a') || 
                               elementUnderMouse?.closest('a[href*="github.com"]') ||
                               elementUnderMouse?.closest('a[href*="linkedin.com"]');
      
      // Only create splash effects if NOT over social links
      if (!isOverSocialLink) {
        let pointer = pointers[0];
        let posX = scaleByPixelRatio(e.clientX);
        let posY = scaleByPixelRatio(e.clientY);
